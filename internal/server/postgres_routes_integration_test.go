...
func TestPostgresBackedPackageAPIAndWebRoutes(t *testing.T) {
	...
	// Fixture value seeded into the throwaway Docker database by this test; it
	// never leaves the container and is not a credential for anything real.
	const apiToken = "packmon-postgres-routes-token" // #nosec G101 -- test fixture for an ephemeral Docker database.
