...
func TestSubsystemLoadersPreserveDefaults(t *testing.T) {
	...
	if retention.PackageCheckStatus != 90*24*time.Hour || retention.Interval != 24*time.Hour {
		t.Fatalf("retention defaults = %#v, want package status 2160h and interval 24h", retention)
	}
