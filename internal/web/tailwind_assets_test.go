...
// Every wide table sits in a .pm-scroll-region, and the links inside those
// tables carry `sr-only` spans, which Tailwind renders as position: absolute.
// If the region is not itself positioned it is not their containing block, so
// overflow-x never clips them: the spans keep their static position far right
// of the visible column and extend the *document* scroll width. The page then
// scrolls sideways into an empty strip on narrow viewports.
func TestScrollRegionEstablishesContainingBlockForAbsoluteChildren(t *testing.T) {
	t.Parallel()

	for _, name := range []string{"tailwind.input.css", "tailwind.css"} {
		css := readTextFile(t, "static", name)
		start := strings.Index(css, ".pm-scroll-region")
		if start < 0 {
			t.Fatalf("%s does not define .pm-scroll-region", name)
		}
		end := strings.Index(css[start:], "}")
		if end < 0 {
			t.Fatalf("%s has an unterminated .pm-scroll-region rule", name)
		}
		rule := css[start : start+end]
		if !strings.Contains(rule, "position:relative") && !strings.Contains(rule, "position: relative") {
			t.Fatalf("%s .pm-scroll-region must be positioned so overflow-x clips absolutely positioned children (sr-only spans):\n%s", name, rule)
		}
	}
}
...
func TestAdminNewAPIKeyFieldUsesContrastSafeBorder(t *testing.T) {
	...
	if !strings.Contains(keys, `class="min-w-0 flex-1 min-h-8 bg-surface border border-accent rounded`) {
...
func TestAdminAPIKeyActionsExplainCredentialImpact(t *testing.T) {
	...
	for _, want := range []string{
		...
		`Deleting permanently removes this revoked credential from the key list; admin audit log entries and scan history remain.`,
		`Create API key`,
...
func TestAdminFormFieldsUseTouchHeight(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			name: "api keys",
			path: []string{"templates", "admin", "keys.html"},
			// The readonly new-api-key reveal field is a compact display
			// control on the admin button 32px system, not a form input.
			markers: []string{
				`id="key-name"`,
				`id="key-expires-in"`,
