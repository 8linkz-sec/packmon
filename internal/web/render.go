...
func adminPrimaryButtonClass(extra ...string) string {
	return joinClassTokens(
		"inline-flex min-h-11 items-center justify-center rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 active:bg-blue-800 pm-focus-ring",
		strings.Join(extra, " "),
	)
}
...
var webMessagesEN = map[webMessageKey]string{
	...
	webMessageKey("dashboard.heading"):                                  "Dashboard",
	...
	webMessageKey("admin.keys.create.expires_label"):                    "Expires At (UTC)",
	webMessageKey("admin.keys.create.expires_title"):                    "Use RFC3339 UTC format, for example %s",
	webMessageKey("admin.keys.create.expires_help"):                     "Required. Use RFC3339 UTC, for example %s. Maximum lifetime is 90 days.",
	...
	webMessageKey("admin.keys.error.expiration_invalid"):                "invalid expiration timestamp; use RFC3339 UTC ending in Z",
	webMessageKey("admin.keys.error.expiration_utc"):                    "expiration must be an RFC3339 UTC timestamp ending in Z",
	webMessageKey("admin.keys.error.expiration_future"):                 "expiration must be in the future",
	webMessageKey("admin.keys.error.expiration_max_lifetime"):           "expiration must be within 90 days",
