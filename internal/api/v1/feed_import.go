...
type FeedImportStore interface {
	...
	ImportVulnerabilityFeedWithAudit(ctx context.Context, feed string, items []db.Vulnerability, deleteIDs []string, status *db.FeedSyncStatus, audit db.FeedImportAuditBuilder) (imported, deleted int, err error)
	...
	ImportMaliciousFeedWithAudit(ctx context.Context, feed string, items []db.MaliciousFinding, deleteIDs []string, status *db.FeedSyncStatus, audit db.FeedImportAuditBuilder) (imported, deleted int, err error)
	...
	ImportVulnCheckWithAudit(ctx context.Context, feed string, entries []db.VulnCheckEntry, status *db.FeedSyncStatus, audit db.FeedImportAuditBuilder) (updated int, err error)
	...
	// Import-Audit Semantics: audit receives updated as imported and 0 deleted.
	ImportCISAKEVWithAudit(ctx context.Context, feed string, cveIDs []string, status *db.FeedSyncStatus, audit db.FeedImportAuditBuilder) (updated int, err error)
	...
	ReplaceCISAKEVWithAudit(ctx context.Context, feed string, cveIDs []string, status *db.FeedSyncStatus, audit db.FeedImportAuditBuilder) (updated, cleared int, err error)
	...
	// Import-Audit Semantics: audit receives updated/imported and cleared/deleted.
	ImportEPSSWithAudit(ctx context.Context, feed string, entries []db.EPSSEntry, status *db.FeedSyncStatus, audit db.FeedImportAuditBuilder) (updated, cleared int, err error)
...
func (h *FeedImportHandler) feedImportAuditBuilder(r *http.Request, feed string) db.FeedImportAuditBuilder {
	return func(imported, deleted int) db.AdminAuditEntry {
		...
		if err != nil || entry == nil {
			// The detail payload could not be built. The import is still audited,
			// with a minimal entry -- an audited action without any audit row is
			// not an option.
