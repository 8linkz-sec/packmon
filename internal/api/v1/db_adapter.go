...
func (a *DBStoreAdapter) EnqueueRefreshWithAudit(ctx context.Context, job *db.RefreshJob, audit db.RefreshEnqueueAuditBuilder) (bool, int, error) {
...
func (a *DBStoreAdapter) ImportVulnerabilityFeedWithAudit(ctx context.Context, feed string, items []db.Vulnerability, deleteIDs []string, status *db.FeedSyncStatus, audit db.FeedImportAuditBuilder) (int, int, error) {
	return a.store.ImportVulnerabilityFeedWithAudit(ctx, feed, items, deleteIDs, status, audit)
}

func (a *DBStoreAdapter) ImportMaliciousFeedWithAudit(ctx context.Context, feed string, items []db.MaliciousFinding, deleteIDs []string, status *db.FeedSyncStatus, audit db.FeedImportAuditBuilder) (int, int, error) {
	return a.store.ImportMaliciousFeedWithAudit(ctx, feed, items, deleteIDs, status, audit)
}

func (a *DBStoreAdapter) ImportVulnCheckWithAudit(ctx context.Context, feed string, entries []db.VulnCheckEntry, status *db.FeedSyncStatus, audit db.FeedImportAuditBuilder) (int, error) {
	return a.store.ImportVulnCheckWithAudit(ctx, feed, entries, status, audit)
}

func (a *DBStoreAdapter) ImportCISAKEVWithAudit(ctx context.Context, feed string, cveIDs []string, status *db.FeedSyncStatus, audit db.FeedImportAuditBuilder) (int, error) {
	return a.store.ImportCISAKEVWithAudit(ctx, feed, cveIDs, status, audit)
}

func (a *DBStoreAdapter) ReplaceCISAKEVWithAudit(ctx context.Context, feed string, cveIDs []string, status *db.FeedSyncStatus, audit db.FeedImportAuditBuilder) (int, int, error) {
	return a.store.ReplaceCISAKEVWithAudit(ctx, feed, cveIDs, status, audit)
}

func (a *DBStoreAdapter) ImportEPSSWithAudit(ctx context.Context, feed string, entries []db.EPSSEntry, status *db.FeedSyncStatus, audit db.FeedImportAuditBuilder) (int, int, error) {
