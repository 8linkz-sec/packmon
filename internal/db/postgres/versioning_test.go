...
func TestExtractFixedVersionFormatsConstraint(t *testing.T) {
	...
	if got := extractFixedVersion("1.5.0", ranges, "npm"); got != ">= 2.0.0" {
		t.Fatalf("extractFixedVersion() = %q, want >= 2.0.0", got)
	}
	// An installed version outside every range still gets the advisory-wide fix.
	if got := extractFixedVersion("", ranges, "npm"); got != ">= 2.0.0" {
		t.Fatalf("extractFixedVersion(no installed version) = %q, want >= 2.0.0", got)
	}
}
