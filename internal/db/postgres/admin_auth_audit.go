...
const (
	...
	//nolint:gosec // G101: SQL statement constant, not a credential.
...
// errNilAdminAuditEntry reports a nil entry reaching the audit writer. The
// builder-driven paths can no longer produce one -- db.FeedImportAuditBuilder
// and db.RefreshEnqueueAuditBuilder return values -- so this now only guards the
// methods that take an *AdminAuditEntry directly.
var errNilAdminAuditEntry = errors.New("postgres: admin audit entry is nil")

// errAnonymousAdminAuditEntry reports an entry without an action. The action is
// the only field that says *what* was audited, and the column is NOT NULL but
// accepts "", so a zero-value entry would otherwise become a nameless audit row
// that records nothing usable. It is the residual invalid state left after the
// builders moved from pointers to values.
var errAnonymousAdminAuditEntry = errors.New("postgres: admin audit entry has no action")

func (s *Store) InsertAdminAuditLog(ctx context.Context, entry *db.AdminAuditEntry) error {
	if entry == nil {
		return fmt.Errorf("%w: %v", db.ErrAdminAuditLog, errNilAdminAuditEntry)
	}
	return withTx(ctx, s.pool, func(tx pgx.Tx) error {
...
func insertAdminAuditLogTx(ctx context.Context, tx pgx.Tx, entry *db.AdminAuditEntry) error {
	// Fail closed: an audited action whose audit row cannot be written must not
	// commit. The callers wrap this into db.ErrAdminAuditLog and roll back, which
	// is the same treatment a failed audit write already gets.
	if entry == nil {
		return errNilAdminAuditEntry
	}
	if strings.TrimSpace(entry.Action) == "" {
		return errAnonymousAdminAuditEntry
	}
