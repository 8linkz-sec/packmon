...
func (s *Store) ImportMaliciousFeedWithAudit(ctx context.Context, feed string, items []db.MaliciousFinding, deleteIDs []string, status *db.FeedSyncStatus, audit func(imported, deleted int) *db.AdminAuditEntry) (int, int, error) {
	return s.importMaliciousFeed(ctx, feed, items, deleteIDs, status, audit)
}

func (s *Store) importMaliciousFeed(ctx context.Context, feed string, items []db.MaliciousFinding, deleteIDs []string, status *db.FeedSyncStatus, audit func(imported, deleted int) *db.AdminAuditEntry) (int, int, error) {
	imported := 0
	deleted := 0
