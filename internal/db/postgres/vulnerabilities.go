...
func (s *Store) FindVulnerabilities(ctx context.Context, ecosystem, name, version string) ([]domain.Finding, error) {
	...
	for rows.Next() {
		...
		fixedVersion := extractFixedVersion(version, versionRangesRaw, ecosystem)
		if version != "" {
...
func findingsFromVulnerabilityRow(row vulnerabilityRow, versions []string) []domain.Finding {
	...
	findingForVersion := func(version string) domain.Finding {
		return domain.Finding{
			...
			URL:          primaryURL,
			Resources:    resources,
			FixedVersion: extractFixedVersion(version, row.versionRangesRaw, row.ecosystem),
			Source:       source,
		}
	}
...
func (s *Store) ImportVulnerabilityFeedWithAudit(ctx context.Context, feed string, items []db.Vulnerability, deleteIDs []string, status *db.FeedSyncStatus, audit db.FeedImportAuditBuilder) (int, int, error) {
	return s.importVulnerabilityFeed(ctx, feed, items, deleteIDs, status, audit)
}

func (s *Store) importVulnerabilityFeed(ctx context.Context, feed string, items []db.Vulnerability, deleteIDs []string, status *db.FeedSyncStatus, audit db.FeedImportAuditBuilder) (int, int, error) {
	imported := 0
	deleted := 0
