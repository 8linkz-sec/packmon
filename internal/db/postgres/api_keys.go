...
func (s *Store) PruneDeletedAPIKeys(ctx context.Context, retention time.Duration) (int, error) {
	if retention <= 0 {
		return 0, nil
	}
	return pruneDeletedAPIKeysTx(ctx, s.pool, time.Now().UTC(), retention)
}

func pruneDeletedAPIKeysTx(ctx context.Context, execer postgresExecer, now time.Time, retention time.Duration) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("postgres: prune deleted API keys: %w", err)
	}
