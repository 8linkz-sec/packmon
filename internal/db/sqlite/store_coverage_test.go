...
func TestLocalFindingAndReferenceHelperBranches(t *testing.T) {
	...
	refs := resourceLinksFromVulnerabilityReferences("GHSA-abcd-efgh-ijkl", `[
		...
		{"type":"VULNCHECK","url":"https://vulncheck.com/cve/CVE-2026-0001"},
		{"type":"WEB","url":"bad url"}
	]`)
	if len(refs) != 3 || refs[0].Label != "GHSA" || refs[1].Label != "CVE" || refs[2].Label != "VulnCheck" {
		t.Fatalf("refs = %+v, want GHSA, CVE, VulnCheck", refs)
	}
	if refs[0].URL != "https://github.com/acme/repo/security/advisories/GHSA-abcd-efgh-ijkl" {
		t.Fatalf("refs[0] = %+v, want matching GHSA advisory URL", refs[0])
	}
