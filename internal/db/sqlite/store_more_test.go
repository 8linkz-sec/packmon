...
func TestStorePathCloseAndFeedConfigNoops(t *testing.T) {
	...
	// The audit-carrying variants are inert here too: feed configuration is a
	// server-side concern, and the local CLI cache has no admin audit log to
	// write to. They must stay silent rather than fail.
	audit := &db.AdminAuditEntry{Action: "feed_save", IP: "127.0.0.1"}
	if err := store.UpsertFeedConfigWithAudit(ctx, &db.FeedConfig{FeedName: "osv"}, audit); err != nil {
		t.Fatalf("UpsertFeedConfigWithAudit() error = %v", err)
	}
	disabledAt := time.Now().UTC()
	if err := store.DeleteFeedConfigWithAudit(ctx, "osv", &disabledAt, audit); err != nil {
