...
//
// Do NOT use this for user-facing fix advice. An advisory that patched a flaw on
// several release branches carries one range per branch, and the lowest fix
// across all of them is below the version the user already runs -- advice that
// reads as "you are already patched" when they are not. Use
// ExtractFixedVersionFor, which selects the fix from the range the installed
// version actually falls in. This function exists as that one's fallback for
// versions outside every range, and is enforced as such by
// TestFixVersionAdviceStaysVersionAware in tests/ci.
