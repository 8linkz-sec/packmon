...
func installFakeDockerExecutable(t *testing.T, dir string) {
	...
	//nolint:gosec // G703: path built from the test's own temporary directory.
	exe, err := os.ReadFile(os.Args[0])
	if err != nil {
		t.Fatalf("read test executable: %v", err)
	}
	//nolint:gosec // G306: test fixture permissions, no production artefact.
...
func runFakeDocker() {
	...
	_, _ = fmt.Fprint(os.Stdout, os.Getenv("PACKMON_FAKE_DOCKER_STDOUT"))
}
