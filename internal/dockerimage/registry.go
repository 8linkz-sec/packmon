...
// ErrRegistryUnsupported marks a client-side policy rejection -- an
// unallowlisted registry host, an unsupported token realm, an insecure or
// unrecognized URL scheme, a missing host, userinfo in a registry URL, a
// disallowed mirror host, or a resolved private address -- that never
// attempted a network request to the registry. Callers use this to tell a
// deliberate non-lookup apart from a genuine registry or network failure, so
// it must always be wrapped alongside ErrDigestUnavailable, never in place of
// it.
var ErrRegistryUnsupported = errors.New("docker registry not supported for digest lookup")
...
func (c *RegistryClient) validateRegistryURL(ctx context.Context, raw string, mirrored bool) error {
	...
	if !isAllowedDockerRegistryHost(host) {
		return fmt.Errorf("%w: %w: unsupported docker registry host %s", ErrDigestUnavailable, ErrRegistryUnsupported, host)
	}
...
func (c *RegistryClient) validateTokenRealm(ctx context.Context, u *url.URL, registryHost string) error {
	if mirrorBase, ok := c.registryMirrorBaseURL(registryHost); ok {
		...
		if host != mirrorHost {
			return fmt.Errorf("%w: %w: unsupported docker registry token realm host %s", ErrDigestUnavailable, ErrRegistryUnsupported, host)
		}
		return nil
	}
	host, err := normalizeHTTPURLTarget(u, c.InsecureHTTP)
...
func normalizeDockerMirrorURLTarget(u *url.URL) (string, error) {
	...
	if strings.EqualFold(u.Scheme, "http") && !dockerMirrorHostIsLoopback(u.Hostname()) {
		return "", fmt.Errorf("%w: %w: insecure registry URL scheme %q", ErrDigestUnavailable, ErrRegistryUnsupported, u.Scheme)
	}
	if ip := net.ParseIP(strings.Trim(u.Hostname(), "[]")); ip != nil && dockerMirrorIPBlocked(ip) {
		return "", fmt.Errorf("%w: %w: registry mirror host %s is not allowed", ErrDigestUnavailable, ErrRegistryUnsupported, host)
	}
	return host, nil
}
...
func normalizeHTTPURLTarget(u *url.URL, allowHTTP bool) (string, error) {
	if u == nil || u.Host == "" {
		return "", fmt.Errorf("%w: %w: missing registry host", ErrDigestUnavailable, ErrRegistryUnsupported)
	}
	if u.User != nil {
		return "", fmt.Errorf("%w: %w: registry URL must not include userinfo", ErrDigestUnavailable, ErrRegistryUnsupported)
	}
	...
	default:
		return "", fmt.Errorf("%w: %w: unsupported registry URL scheme %q", ErrDigestUnavailable, ErrRegistryUnsupported, u.Scheme)
	}
...
func (c *RegistryClient) rejectPrivateHTTPURLTarget(ctx context.Context, host string) error {
	...
	for _, ip := range ips {
		if isPrivateRegistryIP(ip) {
			return fmt.Errorf("%w: %w: registry host %s resolves to private address %s", ErrDigestUnavailable, ErrRegistryUnsupported, host, ip.String())
		}
	}
	return nil
}
