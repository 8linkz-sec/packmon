...
func (s *Store) EnqueueRefreshWithAudit(_ context.Context, job *db.RefreshJob, audit func(created bool, position int) *db.AdminAuditEntry) (bool, int, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if audit != nil {
		if err := s.insertAdminAuditLogLocked(audit(created, position)); err != nil {
