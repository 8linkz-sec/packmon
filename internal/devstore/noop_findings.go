	...
	for _, vuln := range s.vulnerable {
		for _, pkg := range vuln.AffectedPackages {
			...
			findings = append(findings, domain.Finding{
				...
				AdvisoryID: vuln.ID,
				Title:      title,
				FixedVersion: versionpkg.ExtractFixedVersionConstraintFor(
					version, string(pkg.VersionRanges), ecosystem,
				),
				Source: source,
			})
		}
...
func (s *Store) ImportVulnerabilityFeedWithAudit(_ context.Context, feed string, items []db.Vulnerability, deleteIDs []string, status *db.FeedSyncStatus, audit db.FeedImportAuditBuilder) (int, int, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Store) ImportMaliciousFeedWithAudit(_ context.Context, feed string, items []db.MaliciousFinding, deleteIDs []string, status *db.FeedSyncStatus, audit db.FeedImportAuditBuilder) (int, int, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
