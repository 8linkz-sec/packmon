...
func (s *Store) ImportVulnCheckWithAudit(_ context.Context, _ string, _ []db.VulnCheckEntry, status *db.FeedSyncStatus, audit func(imported, deleted int) *db.AdminAuditEntry) (int, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if status != nil {
	...
	if audit != nil {
		if err := s.insertAdminAuditLogLocked(audit(0, 0)); err != nil {
			return 0, err
		}
	}
	return 0, nil
...
func (s *Store) ImportCISAKEVWithAudit(_ context.Context, _ string, _ []string, status *db.FeedSyncStatus, audit func(imported, deleted int) *db.AdminAuditEntry) (int, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if status != nil {
		if err := s.upsertFeedSyncStatusLocked(status); err != nil {
...
func (s *Store) ReplaceCISAKEVWithAudit(_ context.Context, _ string, _ []string, status *db.FeedSyncStatus, audit func(imported, deleted int) *db.AdminAuditEntry) (int, int, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if status != nil {
		if err := s.upsertFeedSyncStatusLocked(status); err != nil {
...
func (s *Store) ImportEPSSWithAudit(_ context.Context, _ string, _ []db.EPSSEntry, status *db.FeedSyncStatus, audit func(imported, deleted int) *db.AdminAuditEntry) (int, int, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if status != nil {
		if err := s.upsertFeedSyncStatusLocked(status); err != nil {
