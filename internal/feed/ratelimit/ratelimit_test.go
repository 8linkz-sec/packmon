...
func TestSetLimitKeepsFullBucketsFullAndCapsOverLimitBuckets(t *testing.T) {
	...
	// Three separate acquisitions: each consumes a token, so the calls must not
	// be chained behind short-circuiting operators.
	for i := range 3 {
		if !partiallyUsed.Acquire() {
			t.Fatalf("failed to consume setup token %d", i+1)
		}
	}
