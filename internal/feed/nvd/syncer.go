...
func (s *Syncer) Sync(ctx context.Context, store db.Store) (*feed.SyncResult, error) {
	...
	for {
		...
		for _, cveID := range cveIDs {
			...
			score, severity, fetchErr := s.fetchCVSSWithRateLimitRetry(ctx, cveID, &rateLimiter)
			...
			if fetchErr != nil {
				...
				// Counted separately from skipped: a CVE that NVD refused is a
				// failure to retry later, not a CVE that legitimately carries
				// no CVSS data. Folding both into one number told operators
				// "completed, skipped=N" for a run that had hard errors.
				progress.failed++
				continue
			}
	...
	// A run with per-CVE failures is not a completed run. Logging it at INFO
	// as "completed" made a failing sync look successful to anyone grepping
	// or alerting on that line.
	completionMessage := "NVD enrichment sync completed"
	logCompletion := s.logger.Info
	if progress.failed > 0 {
		completionMessage = "NVD enrichment sync finished with errors"
		logCompletion = s.logger.Warn
	}
	logCompletion(completionMessage,
	...
	if err := operationErrors.err(); err != nil {
		// A rate-limited run must not be retried on the manager's short
		// backoff: the first retry lands 5s after a run that already spent an
		// hour hitting NVD's limit, so it hits the same wall and only adds
		// load. Record the error and wait for the next scheduled sync, which
		// resumes where this one left off -- enriched CVEs drop out of the
		// discovery query.
		if operationErrors.rateLimited {
			return nil, feed.NonRetryableError(err)
		}
		return nil, err
	}
...
// noteWindowElapsed records that the caller already idled for at least one
// rate-limit window outside the limiter, which happens after a 429 forces a
// Retry-After pause. Without it the limiter would still believe its budget is
// spent and add a second full window on the next request.
func (l *nvdRequestRateLimiter) noteWindowElapsed() {
	l.sent = 0
}
...
type nvdOperationErrorCollector struct {
	total   int
	samples []error
	// rateLimited records whether any failure was an NVD 429. It decides
	// whether the run may be retried immediately or has to wait for the next
	// scheduled sync.
	rateLimited bool
}
...
// fetchCVSSWithRateLimitRetry retries a single CVE lookup across NVD 429
// responses. A 429 is a signal about the whole connection, not about this one
// CVE: the enforced pause is reported back to the shared limiter so the next
// request does not pay a second full window on top of the one just waited out.
func (s *Syncer) fetchCVSSWithRateLimitRetry(ctx context.Context, cveID string, limiter *nvdRequestRateLimiter) (float64, string, error) {
	score, severity, fetchErr := s.fetchCVSS(ctx, cveID)
	for retries := range maxRateLimitRetries {
		var rlErr *rateLimitError
		...
		if limiter != nil && rlErr.retryAfter >= rateLimitWindow {
...
func parseRetryAfterAt(value string, now time.Time) time.Duration {
	...
	if when, err := http.ParseTime(value); err == nil {
		return boundRetryAfter(when.Sub(now))
	}

	return rateLimitWindow
}

// boundRetryAfter clamps an upstream Retry-After hint into a usable delay.
//
// The lower bound is load-bearing. NVD answers 429 with "Retry-After: 0" (and
// occasionally with a timestamp already in the past). Taking that literally
// means retrying instantly against a server that just refused the request,
// which burns every retry inside a few milliseconds and drops the CVE. A
// non-positive hint carries no information, so it is treated exactly like a
// missing header: wait one full rate-limit window.
