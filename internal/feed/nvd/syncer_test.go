...
func TestSync_RetriesRateLimitedCVE(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if requestCount.Add(1) == 1 {
			// One second, not zero: a non-positive hint now floors to a full
			// rate-limit window, which would make this test sleep 30s.
...
// A run that NVD rate-limited must not come back on the manager's 5s backoff:
// the retry would hit the same limit and only add load. It also must not report
// its failures as "skipped", which made a failing sync read as a clean one.
func TestSync_RateLimitedRunIsNotRetriedImmediately(t *testing.T) {
	t.Parallel()
	...
	store := &nvdStoreStub{cveIDs: []string{"CVE-2025-42424"}}
	syncer := NewSyncer(slog.New(slog.NewTextHandler(io.Discard, nil)),
		WithAPIURL(srv.URL),
		WithHTTPClient(srv.Client()),
	)
	...
	result, err := syncer.Sync(context.Background(), store)
	if err == nil {
		t.Fatalf("Sync() error = nil, want rate-limit failure (result %+v)", result)
	}
	if !feed.IsNonRetryableError(err) {
		t.Fatalf("Sync() error = %v, want a non-retryable error so the manager waits for the next scheduled sync", err)
	}
	if got := int(requestCount.Load()); got != maxRateLimitRetries+1 {
		t.Fatalf("request count = %d, want %d (initial + %d retries)", got, maxRateLimitRetries+1, maxRateLimitRetries)
	}
}
...
func TestFetchCVSSWithRateLimitRetryRetriesTypedRateLimit(t *testing.T) {
	...
	limiter := &nvdRequestRateLimiter{batchSize: batchSizeNoKey, sent: batchSizeNoKey}
	score, severity, err := syncer.fetchCVSSWithRateLimitRetry(context.Background(), "CVE-2025-51515", limiter)
	if err != nil {
...
func TestFetchCVSSErrorBranchesAndRetryHelpers(t *testing.T) {
	...
	_, _, err = syncer.fetchCVSSWithRateLimitRetry(context.Background(), "CVE-1", nil)
	...
	// A hint that already expired carries no information. Waiting zero would
	// retry instantly against a server that just refused the request, so it
	// falls back to a full rate-limit window like a missing header does.
	if got := parseRetryAfterAt(past, now); got != rateLimitWindow {
		t.Fatalf("parseRetryAfterAt(past date) = %v, want %v", got, rateLimitWindow)
	}
	if got := parseRetryAfter("0"); got != rateLimitWindow {
		t.Fatalf("parseRetryAfter(zero) = %v, want %v", got, rateLimitWindow)
	}
