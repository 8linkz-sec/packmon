...
func TestManagerStartHonorsDisabledStartupSync(t *testing.T) {
	...
	if err := manager.Register(FeedConfig{Syncer: syncer, Enabled: true, Mode: FeedModeSelf}); err != nil {
		t.Fatalf("Register() error = %v", err)
	}
...
func TestRecordRunningStatusPreservesExistingMetadata(t *testing.T) {
	...
	_ = manager.recordRunningStatus(context.Background(), "osv")
	metadata[0] = '['
...
func TestRecordStatusPreservesExistingDataForNonSuccessStatuses(t *testing.T) {
	...
	_ = manager.recordStatus(context.Background(), "ghsa", "external", "", 0, nil)
	metadata[0] = '['
	...
	_ = manager.recordStatus(context.Background(), "ghsa", "error", "upstream failed", time.Second, nil)
...
func TestRegisterNilSyncerIsIgnored(t *testing.T) {
	...
	// Invalid config: the rejection is the point, the feed count is asserted below.
	_ = m.Register(FeedConfig{Syncer: nil, Enabled: true})
...
func TestRegisterWithIntervalOverridesDefault(t *testing.T) {
	...
	store := &managerStoreStub{}
	m := NewManager(store, nil, time.Hour)
	// Empty config is expected to be rejected; the feed count is asserted below.
	_ = m.RegisterWithInterval(FeedConfig{}, time.Minute)
	if len(m.feeds) != 0 {
		t.Fatalf("feeds count = %d, want 0 after nil RegisterWithInterval", len(m.feeds))
	}

	syncer := &permanentSyncerStub{name: "fast-feed"}
	if err := m.RegisterWithInterval(FeedConfig{
		Syncer:  syncer,
		Mode:    FeedModeSelf,
		Enabled: true,
	}, 5*time.Minute); err != nil {
		t.Fatalf("RegisterWithInterval() error = %v", err)
	}
...
func TestStartSkipsDisabledAndExternalFeeds(t *testing.T) {
	...
	if err := m.Register(FeedConfig{Syncer: disabledSyncer, Mode: FeedModeSelf, Enabled: false}); err != nil {
		t.Fatalf("Register() error = %v", err)
	}
	if err := m.Register(FeedConfig{Syncer: externalSyncer, Mode: FeedModeExternal, Enabled: true}); err != nil {
		t.Fatalf("Register() error = %v", err)
	}
