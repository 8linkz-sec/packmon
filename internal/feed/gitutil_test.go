...
// A clone killed by its deadline leaves a directory that already contains
// .git, which isCloned() would accept. Every later sync would then fetch and
// reset against that incomplete work tree instead of cloning again, so the
// feed can never recover on its own.
func TestGitRepoCloneUsesNoProgressAndClearsPartialCheckout(t *testing.T) {
	oldGitExecutable := gitExecutable
	oldGitExecutableArgs := gitExecutableArgs
	gitExecutable = os.Args[0]
	gitExecutableArgs = []string{"-test.run=^TestGitRepoFakeGitFailingClone$", "--"}
	t.Cleanup(func() {
		gitExecutable = oldGitExecutable
		gitExecutableArgs = oldGitExecutableArgs
	})
	t.Setenv("PACKMON_FAKE_GIT_FAILING_CLONE", "1")

	parent := t.TempDir()
	argsFile := filepath.Join(parent, "clone-args.txt")
	t.Setenv("PACKMON_FAKE_GIT_ARGS_FILE", argsFile)
	...
	if err := repo.clone(context.Background()); err == nil {
		t.Fatal("clone(failing git) error = nil")
	}

	// #nosec G304 -- test-only read of a path this test created under its own TempDir.
	recorded, err := os.ReadFile(argsFile)
	if err != nil {
		t.Fatalf("read recorded clone args: %v", err)
	}
	if !strings.Contains(string(recorded), "--no-progress") {
		t.Fatalf("clone args = %q, want --no-progress so per-percent progress does not fill the stored feed error", recorded)
	}
	if _, err := os.Stat(repo.Dir); !os.IsNotExist(err) {
		t.Fatalf("partial checkout survived the failed clone (stat error = %v)", err)
	}
}

func TestGitRepoCloneTimeoutIsIndependentOfCommandTimeout(t *testing.T) {
	t.Parallel()

	// A CommandTimeout tuned for incremental fetch/reset work must not
	// shorten the initial clone, which transfers an entire repository.
	repo := &GitRepo{CommandTimeout: 20 * time.Millisecond}
	if got := repo.cloneTimeout(); got != defaultGitCloneTimeout {
		t.Fatalf("cloneTimeout() with CommandTimeout set = %s, want %s", got, defaultGitCloneTimeout)
	}
	if got := repo.effectiveCommandTimeout(); got != 20*time.Millisecond {
		t.Fatalf("effectiveCommandTimeout() = %s, want the configured 20ms", got)
	}

	repo.CloneTimeout = time.Minute
	if got := repo.cloneTimeout(); got != time.Minute {
		t.Fatalf("cloneTimeout() with CloneTimeout set = %s, want 1m", got)
	}
}

func TestGitRepoFakeGitFailingClone(t *testing.T) {
	if os.Getenv("PACKMON_FAKE_GIT_FAILING_CLONE") != "1" {
		t.Skip("helper process only")
	}
	var gitArgs []string
	for i, arg := range os.Args {
	...
	if len(gitArgs) == 0 || gitArgs[0] != "clone" {
		_, _ = os.Stderr.WriteString("fake git expected a clone invocation\n")
		os.Exit(2)
	}
	if file := os.Getenv("PACKMON_FAKE_GIT_ARGS_FILE"); file != "" {
		// #nosec G703 -- helper process writes to the path the parent test set for it.
		_ = os.WriteFile(file, []byte(strings.Join(gitArgs, " ")), 0o600)
	}
	// Stand in for a clone killed during checkout: the target directory and
	// its .git already exist by the time the process dies.
	target := gitArgs[len(gitArgs)-1]
	// #nosec G703 -- clone target is supplied by the parent test, not by user input.
	_ = os.MkdirAll(filepath.Join(target, ".git"), 0o750)
	os.Exit(1)
}
