...
func TestAdminKeysPageConfirmationsIncludeKeyIdentity(t *testing.T) {
	...
	for _, want := range []string{
		...
		fmt.Sprintf(`Permanently delete revoked API key <bdi dir="auto">n8n-scanner</bdi> (ID %d)?`, revokedID),
		"Confirm revoke",
		"Confirm delete",
		`data-submit-lock`,
		`data-submit-lock-button`,
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			want: []string{
				...
				fmt.Sprintf(`aria-label="Permanently delete revoked API key n8n-scanner (ID %d)"`, revokedID),
				`min-h-11`,
				`px-3 py-2`,
			},
		},
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			checks: []struct {
				...
				{tag: "details", marker: fmt.Sprintf(`aria-label="Permanently delete revoked API key n8n-scanner (ID %d)"`, revokedID)},
			},
		},
		{
			name:   "feeds",
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			summaries: []string{
				...
				fmt.Sprintf("Permanently delete revoked API key n8n-scanner (ID %d)", revokedID),
			},
		},
		{
