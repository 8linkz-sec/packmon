...
func TestRunAuditRetentionOnceSkipsDisabledDurations(t *testing.T) {
	...
	if store.scanCalls != 0 || store.adminCalls != 0 || store.queueCalls != 0 || store.packageCheckStatusCalls != 0 || store.reputationCalls != 0 {
		t.Fatalf("retention calls = scan:%d admin:%d queue:%d package_check_status:%d reputation:%d, want none when durations are disabled", store.scanCalls, store.adminCalls, store.queueCalls, store.packageCheckStatusCalls, store.reputationCalls)
	}
}
