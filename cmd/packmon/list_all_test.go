...
func TestRunListAll_UnknownLatestForUnsupportedEcosystemNoNetwork(t *testing.T) {
	isolatedListAllEnv(t)
	dir := t.TempDir()
	// pubspec.lock -> pub ecosystem, which fetchLatestVersion does not support.
...
`)
	...
	if !contains(out, "unknown") {
		t.Errorf("expected LATEST=unknown for unsupported ecosystem:\n%s", out)
	}
	if !called {
