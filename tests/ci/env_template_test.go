...
func TestReadmeDockerQuickStartUsesInitSecretsFlow(t *testing.T) {
	...
	// Reader-first ordering. A newcomer must learn what Packmon is and how to
	// get a binary before meeting the source/Docker walkthrough, which is the
	// only path that needs init-secrets.
	orderedReadmeMarkers := []string{
	...
	for i := 0; i < len(orderedReadmeMarkers)-1; i++ {
		assertSubstringOrder(t, text, orderedReadmeMarkers[i], orderedReadmeMarkers[i+1])
	}
