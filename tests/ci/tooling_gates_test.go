...
func TestGitHubCILintJobRunsNonGoToolingGates(t *testing.T) {
	t.Parallel()

	var wf struct {
		Jobs map[string]workflowJob `yaml:"jobs"`
	}
	if err := yaml.Unmarshal([]byte(readRepoFile(t, filepath.Join(".github", "workflows", "ci.yml"))), &wf); err != nil {
		t.Fatalf("parse ci.yml: %v", err)
	}

	lintJob, ok := wf.Jobs["lint"]
	if !ok {
		t.Fatal("ci workflow has no lint job")
	}
	runs := joinedStepRuns(lintJob.Steps)
	for _, want := range []string{
		...
		"scripts/*.sh scripts/lib/*.sh deploy/n8n/*.sh",
		...
		if !strings.Contains(runs, want) {
			t.Fatalf("ci lint job missing tooling gate marker %q", want)
		}
	}
}
