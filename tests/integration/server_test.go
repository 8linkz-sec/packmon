...
func scanServerStdout(r io.Reader, logs *serverProcessLogs, addrCh chan<- serverBoundAddr) {
	...
	for scanner.Scan() {
		...
		// The main ready line embeds the dashboard URL in its message, so it
		// cannot be matched by a fixed string. Key off the dashboard_url field
		// instead, which only that line carries.
		if _, isMain := entry["dashboard_url"].(string); isMain {
			addrCh <- serverBoundAddr{kind: "main", addr: addr}
			continue
		}
		if msg == "metrics server listening" {
