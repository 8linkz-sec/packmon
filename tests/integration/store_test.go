...
func TestPostgresAPIKeyExpirationRevocationAndDeletion(t *testing.T) {
	...
	// admin audit log keeps the deletion traceable, and scan_log rows retain
	// their history with api_key_id cleared by ON DELETE SET NULL.
	for i := range keys {
		if keys[i].ID == keyID {
			t.Fatalf("ListAPIKeys(after delete) still returns %+v, want the row permanently removed", keys[i])
		}
	}
}
